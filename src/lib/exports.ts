...
export const salesToCsv = (sales: Sale[]): string => {
  ...
  const rows = sales.map((sale) => [
    ...
    sale.date,
    sale.client_or_tx,
    toValue(sale.transaction_ref),
    sale.channel,
    toValue(sale.customer_country),
    sale.product_ref,
    toValue(sale.quantity),
    toValue(sale.sell_price_unit_ht),
    toValue(sale.sell_price_unit_ttc),
    toValue(sale.sell_total_ht),
    toValue(sale.shipping_charged),
    toValue(sale.shipping_charged_ttc),
    toValue(sale.shipping_real),
    toValue(sale.shipping_real_ttc),
